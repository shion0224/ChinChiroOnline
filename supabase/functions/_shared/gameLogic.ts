/**
 * チンチロのゲームロジック（サーバー側）
 * クライアント側の gameLogic.js と同等のロジックをTypeScriptで実装
 */
...
export function rollThreeDice(): [number, number, number] {
  const dice = [rollDice(), rollDice(), rollDice()].sort(
    (a, b) => a - b
...
/**
 * DB上の hand_type（表示名）から強さを算出
 */
export function getHandStrengthFromDisplayName(
  handType: string,
  handValue: number
): number {
